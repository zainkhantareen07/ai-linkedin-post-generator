...
# Setup path to recognize local imports correctly
...
<style>
    ...
    /* Bullet point chips for hashtags */
    .hashtag-chip {
    ...
    /* Streamlit button layout refinements */
    .stButton > button {
        border-radius: 8px;
        font-weight: 600;
        transition: all 0.2s;
...
with st.sidebar:
    ...
    st.markdown(
        "Powered by **Groq Cloud (Llama-3.3 70B)**. Super-fast, professional, and optimized content generator."
    )
...
with col1:
    topic = st.text_input(
        ...
        placeholder="e.g. Why I quit my job to build a startup, lessons from failing my first product launch...",
...
if generate_btn:
    ...
    if not valid:
        ...
        try:
            ...
            # Add to generation history stack
...
if st.session_state.post:
    ...
    for ex in examples:
        if ex and st.button(f"→ {ex}", key=ex):
